...
export function EnvStep({ data, update }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3 p-3 rounded-lg bg-surface-base border border-border-subtle">
        <Lock className="w-4 h-4 text-accent mt-0.5" />
        <div>
          <p className="text-[13px] font-medium text-text-primary">Environment variables</p>
          <p className="text-[11px] text-text-secondary mt-px">
            Encrypted at rest and injected into the bot process. Optional.
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={add}
          className="px-3 py-2 rounded-lg bg-accent text-accent-fg hover:bg-accent-hover transition-colors"
        >
      ...
      {error && (
        <div className="flex items-center gap-2 text-[11px] text-status-crashed">
      ...
      {entries.length === 0 ? (
        <div className="text-center py-6">
          <p className="text-[13px] text-text-muted">No environment variables yet</p>
        </div>
      ) : (
        <div className="space-y-1">
          {entries.map(([k, v]) => (
            ...
            >
              ...
              <span className="flex-1 text-[11px] text-text-secondary truncate">{v}</span>
              <button
                onClick={() => remove(k)}
